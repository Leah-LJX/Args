...
public class PathFinder {
	...
	public PathFinder(PetriNet p, List<Place> inits, Place goal, int timeline, int tokens) throws IOException {
	...
	public PathFinder(PetriNet p, List<Place> inits, Place goal, int timeline, int tokens, Set<String> white,
			List<String> clones) throws IOException {
		solver = new Solver();
		encoding = new PetrinetEncoding(p, inits, goal);
		encoding.setBound(timeline, tokens);
	...
	public PathFinder(PetriNet p, List<Place> inits, Place goal, int timeline, int tokens, List<String> clones,
			Option objOption, int solverLimit) throws IOException {
		...
		try {
			solver.build(encoding);//将对encoding做的编码规则转换成底层的solver里面的东西
			OK = true;
		} catch (ContradictionException e) {
			OK = false;
		}
