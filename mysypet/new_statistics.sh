...
function build-csv {
    ...
    echo "ID,Soot Time,Sketch Generation Time,Sketch Completion Time,Compilation Time,Running Test Cases Time,Total Time,Paths,Programs, Apis" > $filename
    for f in {1..50} ; do
        id[$f]=$f
    done
    ...
    for f in google ; do 
        for z in output/$f/*.log ; do
            y=`grep "Recommend Api Number" $z | cut -d ':' -f 2`
            apis[$i]=`echo "$y" | bc -l`
    ...
    echo "ID,Soot Time,Sketch Generation,Sketch Completion,Compilation,Running Test Cases,Total Time,Paths,Programs,Apis" > $filename
    for f in {1..50} ; do 
        echo "${id[$f]},${soot[$f]},${sketch_gen[$f]},${sketch_comple[$f]},${compilation[$f]},${test_cases[$f]},${total[$f]},${sketch[$f]},${program[$f]},${apis[$f]}" >> $filename
    done  
